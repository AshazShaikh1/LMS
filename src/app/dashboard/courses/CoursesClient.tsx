      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        {role === 'student' ? (
          <div className="flex rounded-lg bg-slate-100 p-1">
            ...
            >
              My Enrolled Courses ({enrolledCount})
            </button>
            <button
            ...
            >
              All Courses Catalog ({allCourses.length})
            </button>
          </div>
        ) : role === 'teacher' ? (
          <div className="flex rounded-lg bg-slate-100 p-1">
            ...
            >
              My Teaching Courses ({teachingCount})
            </button>
            <button
              onClick={() => setActiveTab('catalog')}
              className={`rounded-md px-3.5 py-1.5 text-xs font-semibold transition ${
            ...
            >
              All Institution Courses ({allCourses.length})
            </button>
          </div>
        ) : (
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-300 pl-9 pr-3.5 py-1.5 text-xs text-slate-900 shadow-xs focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
            />
          </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <select
              ...
              className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs text-slate-900 shadow-xs focus:border-indigo-500 focus:outline-none"
            >
              {categories.map((cat) => (
      ...
      {filteredCourses.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-slate-200 bg-white p-12 text-center shadow-xs">
          ...
          {role === 'student' && activeTab === 'enrolled' && (
            <button
              onClick={() => setActiveTab('catalog')}
              className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-100 transition"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCourses.map((course) => {
            ...
            return (
              <div
                key={course.id}
                className="flex flex-col justify-between rounded-xl border border-slate-200 bg-white p-6 shadow-xs hover:border-slate-300 hover:shadow-sm transition"
              >
                <div>
                ...
                <div className="mt-4 pt-2">
                  <Link
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-lg bg-slate-50 px-3.5 py-2 text-xs font-semibold text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 transition"
                  >
